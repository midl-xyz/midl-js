...
import { type AbstractProvider, MaestroProvider } from "~/providers";
...
type ConfigParams = {
     ...
     * @default MaestroProvider
     */
    provider?: AbstractProvider;

    /**
     * API key for MaestroProvider (if used)
     */
                ...
                partialize: (state: ConfigState) => ({
                    ...Object.fromEntries(
                        Object.entries(state).filter(
                            ([key]) =>
                                shouldPersist &&
                                ["network", "connection", "accounts"].includes(
                                    key
                                )
                        )
                    ),
